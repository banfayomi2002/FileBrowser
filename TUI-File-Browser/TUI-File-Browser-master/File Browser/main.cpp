...
void ProcessEvents(FileView& view, FileModel& model) {
	while (!view.GetQuitState()) {
		...
		switch (e.GetType())
		{
			case ConsoleAPI::Event::EventType::KEY: view.ProcessKeyEvent(e.GetKeyboardEvent(), model); break;
			case ConsoleAPI::Event::EventType::MOUSE: view.ProcessMouseEvent(e.GetMouseEvent(), model); break;
		}
	}
}
